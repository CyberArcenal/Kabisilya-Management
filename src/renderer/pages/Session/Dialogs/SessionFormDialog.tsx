...
import {
    X, Save, Calendar, AlertCircle, Loader,
    CheckCircle, Info, CalendarDays, Clock
} from 'lucide-react';
...
const SessionFormDialog: React.FC<SessionFormDialogProps> = ({
    ...
    const handleChange = async (field: keyof FormData, value: string | number) => {
        const newFormData = {
            ...formData,
            [field]: value
        };
        ...
        if (field === 'name' && typeof value === 'string' && value.trim().length > 0) {
            try {
                ...
                const uniqueValidation = await sessionAPI.checkNameExists(value, newFormData.year, mode === 'edit' ? id : undefined);

                setValidationState({
                    isValid: formatValidation.data,
                    isUnique: uniqueValidation.data,
                    message: formatValidation.data ?
                        (uniqueValidation.data ? 'Name is available' : 'Name already exists for this year') :
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if(!await dialogs.confirm({title: mode === 'add' ? 'Create Session' : 'Update Session', message: `Are you sure you want to ${mode === 'add' ? 'create' : 'update'} this session?`}))return;
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div className="text-xs text-gray-600">
                                {mode === 'add' ? 'Add a new farming season' : 'Update session details'}
                            </div>
                        </div>
                    </div>
                    <button
                ...
                <div className="p-6">
                    {loading ? (
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            {/* Year Field */}
                            <div>
                                <label className="block text-xs font-medium mb-1.5 text-gray-700" htmlFor="year">
                                ...
                                <select
                                    ...
                                    className={`w-full px-3 py-2 rounded text-sm border ${errors.year ? 'border-red-500' : 'border-gray-300'
                                        } focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none`}
                                    disabled={submitting}
                                >
                            ...
                            {/* Season Type Field */}
                            <div>
                                <label className="block text-xs font-medium mb-1.5 text-gray-700" htmlFor="seasonType">
                                    Season Type
                                </label>
                                <select
                                    id="seasonType"
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-medium mb-1.5 text-gray-700" htmlFor="startDate">
                                    ...
                                    <input
                                        ...
                                        className={`w-full px-3 py-2 rounded text-sm border ${errors.startDate ? 'border-red-500' : 'border-gray-300'
                                            } focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none`}
                                        required
                                        disabled={submitting}
                                    />
                                ...
                                <div>
                                    <label className="block text-xs font-medium mb-1.5 text-gray-700" htmlFor="endDate">
                                        End Date (Optional)
                                    </label>
                                    <input
                                        id="endDate"
                                        ...
                                        className={`w-full px-3 py-2 rounded text-sm border ${errors.endDate ? 'border-red-500' : 'border-gray-300'
                                            } focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none`}
                                        disabled={submitting}
                            ...
                            {mode === 'edit' && (
                                <div>
                                    <label className="block text-xs font-medium mb-1.5 text-gray-700" htmlFor="status">
                                        Status
                                    </label>
                                    <select
                                        id="status"
