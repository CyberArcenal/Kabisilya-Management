// components/Debt/components/DebtActionsDropdown.tsx
import React, { useRef, useEffect, useState } from 'react';
import {
    Eye, Edit, Trash2, DollarSign, FileText,
    CheckCircle, XCircle, AlertTriangle, MoreVertical,
    Clock, Percent, Check, X, History, CreditCard
} from 'lucide-react';
                ...
                >
                    <div className="py-2">
                        ...
                        {debt.balance > 0 && debt.status !== 'cancelled' && (
                            <button
                                onClick={() => handleAction(onMakePayment)}
                                className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50"
                            >
                        ...
                        {debt.status === 'pending' && (
                            <>
                                ...
                                <button
                                    onClick={() => handleAction(() => onUpdateStatus('cancelled'))}
                                    className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50"
                                >
                        ...
                        {debt.status === 'partially_paid' && (
                            <>
                                <button
                                    onClick={() => handleAction(() => onUpdateStatus('paid'))}
                                    className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50"
                                >
                                ...
                                <button
                                    onClick={() => handleAction(() => onUpdateStatus('overdue'))}
                                    className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50"
                                >
                                    <AlertTriangle className="w-4 h-4" style={{ color: 'rgb(220, 38, 38)' }} />
