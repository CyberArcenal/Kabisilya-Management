...
class BukidAnalyticsHandler {
  ...
  async getBukidOverview(repositories, params) {
    try {
      ...
      // Total bukids
      const totalBukids = await bukid.count();
      
      // Active bukids (with active pitaks)
      const activeBukids = await bukid
      ...
      const pitaksPerBukid = await bukid
        .createQueryBuilder("b")
        .leftJoin("b.pitaks", "p")
      ...
      const luwangPerBukid = await assignment
      ...
      return {
        ...
        data: {
          ...
          distribution: pitaksPerBukid.map((/** @type {{ b_id: any; b_name: any; pitakCount: string; }} */ b) => ({
            bukidId: b.b_id,
            bukidName: b.b_name,
            pitakCount: parseInt(b.pitakCount) || 0,
          })),
          production: luwangPerBukid.map((/** @type {{ b_id: any; b_name: any; totalLuwang: string; }} */ b) => ({
            bukidId: b.b_id,
            bukidName: b.b_name,
  ...
  async getBukidDetails(repositories, params) {
    try {
      ...
      const pitakStats = pitaks.map((/** @type {{ assignments: any[]; id: any; location: any; status: any; }} */ p) => {
        const completedAssignments = p.assignments.filter((/** @type {{ status: string; }} */ a) => a.status === "completed");
        const activeAssignments = p.assignments.filter((/** @type {{ status: string; }} */ a) => a.status === "active");
        ...
        return {
          ...
          totalLuwang,
          workers: [...new Set(p.assignments.map((/** @type {{ worker: { name: any; }; }} */ a) => a.worker?.name).filter(Boolean))],
        };
      });

      // Get assignments for this bukid
      const assignments = await assignment.find({
        where: { pitak: { bukid: { id: bukidId } } },
      ...
      const financialStats = {
        ...
        totalDeductions: payments.reduce((/** @type {number} */ sum, /** @type {{ manualDeduction: string; }} */ p) => sum + (parseFloat(p.manualDeduction) || 0), 0),
        byStatus: payments.reduce((/** @type {{ [x: string]: any; }} */ acc, /** @type {{ status: string | number; }} */ p) => {
      ...
      return {
        ...
        data: {
          ...
          workers: workers.map((/** @type {{ w_id: any; w_name: any; w_status: any; assignmentCount: string; totalLuwang: string; }} */ w) => ({
            id: w.w_id,
            name: w.w_name,
            status: w.w_status,
          ...
          summary: {
            ...
            activePitaks: pitaks.filter((/** @type {{ status: string; }} */ p) => p.status === "active").length,
            totalWorkers: workers.length,
            totalLuwang: pitakStats.reduce((/** @type {any} */ sum, /** @type {{ totalLuwang: any; }} */ p) => sum + p.totalLuwang, 0),
            totalPayments: payments.length,
          },
        },
      };
  ...
  async getBukidProductionTrend(repositories, params) {
    try {
      // @ts-ignore
      const { bukidId, startDate, endDate, interval = "monthly" } = params;
      // @ts-ignore
      ...
      return {
        ...
        data: {
          interval,
          trend: trendData.map((/** @type {{ period: any; totalLuwang: string; assignmentCount: string; }} */ row) => ({
            period: row.period,
            totalLuwang: parseFloat(row.totalLuwang) || 0,
            assignmentCount: parseInt(row.assignmentCount) || 0,
            averageLuwang: parseFloat(row.totalLuwang) / parseInt(row.assignmentCount) || 0,
          })),
          summary: {
            totalPeriods: trendData.length,
            totalLuwang: trendData.reduce((/** @type {number} */ sum, /** @type {{ totalLuwang: string; }} */ row) => sum + parseFloat(row.totalLuwang), 0),
            totalAssignments: trendData.reduce((/** @type {number} */ sum, /** @type {{ assignmentCount: string; }} */ row) => sum + parseInt(row.assignmentCount), 0),
          },
        },
      };
    } catch (error) {
  ...
  async getBukidWorkerDistribution(repositories, params) {
    try {
      ...
      const workerDistribution = await pitak
      ...
      const pitakDistribution = await worker
        .createQueryBuilder("w")
      ...
      return {
        ...
        data: {
          workersPerPitak: workerDistribution.map((/** @type {{ p_id: any; p_location: any; p_status: any; workerCount: string; workerNames: string; }} */ p) => ({
            pitakId: p.p_id,
          ...
          pitaksPerWorker: pitakDistribution.map((/** @type {{ w_id: any; w_name: any; w_status: any; pitakCount: string; pitakLocations: string; }} */ w) => ({
            workerId: w.w_id,
            workerName: w.w_name,
            status: w.w_status,
            pitakCount: parseInt(w.pitakCount) || 0,
            pitakLocations: w.pitakLocations ? w.pitakLocations.split(",") : [],
          })),
          summary: {
            totalPitaks: workerDistribution.length,
            totalWorkers: pitakDistribution.length,
            avgWorkersPerPitak: workerDistribution.reduce((/** @type {number} */ sum, /** @type {{ workerCount: string; }} */ p) => sum + parseInt(p.workerCount), 0) / workerDistribution.length || 0,
            avgPitaksPerWorker: pitakDistribution.reduce((/** @type {number} */ sum, /** @type {{ pitakCount: string; }} */ w) => sum + parseInt(w.pitakCount), 0) / pitakDistribution.length || 0,
          },
        },
      };
    } catch (error) {
  ...
  async getBukidFinancialSummary(repositories, params) {
    try {
      // @ts-ignore
      const { bukidId, startDate, endDate } = params;
      if (!bukidId) {
        throw new Error("bukidId is required");
      }
      ...
      const totalGrossPay = payments.reduce((/** @type {number} */ sum, /** @type {{ p_gross_pay: string; }} */ p) => sum + parseFloat(p.p_gross_pay), 0);
      const totalNetPay = payments.reduce((/** @type {number} */ sum, /** @type {{ p_net_pay: string; }} */ p) => sum + parseFloat(p.p_net_pay), 0);
      const totalDeductions = payments.reduce((/** @type {number} */ sum, /** @type {{ p_manual_deduction: string; }} */ p) => sum + (parseFloat(p.p_manual_deduction) || 0), 0);
      const totalLuwang = parseFloat(assignments?.totalLuwang) || 0;

      const paymentByStatus = payments.reduce((/** @type {{ [x: string]: { amount: number; }; }} */ acc, /** @type {{ p_status: any; p_net_pay: string; }} */ p) => {
      ...
      return {
        ...
        data: {
          payments: payments.map((/** @type {{ p_id: any; p_gross_pay: string; p_manual_deduction: string; p_net_pay: string; p_status: any; p_payment_date: any; workerName: any; pitakLocation: any; }} */ p) => ({
            id: p.p_id,
    ...
    try {
      ...
      const comparisonData = await Promise.all(
        bukidIds.map(async (/** @type {any} */ bukidId) => {
          ...
          // Get pitak count
          const pitakCount = await pitak.count({
            where: { bukid: { id: bukidId } },
          });

          // Get assignment statistics
          const assignmentStats = await assignment
            .createQueryBuilder("a")
            .leftJoin("a.pitak", "p")
            .leftJoin("p.bukid", "b")
            .select([
          ...
          const paymentStats = await payment
            .createQueryBuilder("p")
            .leftJoin("p.pitak", "pt")
            .leftJoin("pt.bukid", "b")
            .select([
          ...
          return {
            ...
            metrics: {
              ...
              activeAssignments: parseInt(assignmentStats?.activeAssignments) || 0,
              ...
              efficiency: (parseFloat(assignmentStats?.totalLuwang) / pitakCount) || 0,
            },
          };
        })
      );
      ...
      // Calculate rankings
      // @ts-ignore
      comparisonData.forEach((/** @type {{ rankings: { [x: string]: { value: any; rank: any; percentile: number; }; }; metrics: { [x: string]: any; }; }} */ bukid, /** @type {any} */ index) => {
        bukid.rankings = {};
        Object.keys(bukid.metrics).forEach(metric => {
          const values = comparisonData.map((/** @type {{ metrics: { [x: string]: any; }; }} */ b) => b.metrics[metric]);
          ...
          bukid.rankings[metric] = {
            value,
            rank: values.sort((/** @type {number} */ a, /** @type {number} */ b) => b - a).indexOf(value) + 1,
      ...
      return {
        ...
        data: {
          ...
          summary: {
            ...
            averagePitaks: comparisonData.reduce((/** @type {any} */ sum, /** @type {{ metrics: { pitaks: any; }; }} */ b) => sum + b.metrics.pitaks, 0) / comparisonData.length,
            averageLuwang: comparisonData.reduce((/** @type {any} */ sum, /** @type {{ metrics: { totalLuwang: any; }; }} */ b) => sum + b.metrics.totalLuwang, 0) / comparisonData.length,
            averageEfficiency: comparisonData.reduce((/** @type {any} */ sum, /** @type {{ metrics: { efficiency: any; }; }} */ b) => sum + b.metrics.efficiency, 0) / comparisonData.length,
          },
        },
      };
    } catch (error) {
  ...
  groupAssignmentsByMonth(assignments) {
    ...
    assignments.forEach((/** @type {{ assignmentDate: string | number | Date; luwangCount: string; }} */ assignment) => {
  ...
  groupPaymentsByMonth(payments) {
    ...
    payments.forEach((/** @type {{ p_payment_date: string | number | Date; p_gross_pay: string; p_net_pay: string; p_manual_deduction: string; }} */ payment) => {
      if (payment.p_payment_date) {
        ...
        // @ts-ignore
        grouped[monthYear].deductions += parseFloat(payment.p_manual_deduction) || 0;
      }
    });
