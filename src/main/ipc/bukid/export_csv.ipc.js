...
module.exports = async function exportBukidToCSV(params = {}) {
  try {
    ...
    if (filters.status) {
      queryBuilder.andWhere("bukid.status = :status", {
    ...
    // @ts-ignore
    const csvData = bukids.map(
      (
        /** @type {{ id: any; name: any; location: any; status: any; pitaks: string | any[]; createdAt: any; updatedAt: any; }} */ bukid,
      ) => ({
        id: bukid.id,
        name: bukid.name,
