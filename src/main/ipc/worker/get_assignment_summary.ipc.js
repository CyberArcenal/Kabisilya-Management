...
module.exports = async function getWorkerAssignmentSummary(params = {}) {
  try {
    // @ts-ignore
    // @ts-ignore
    const { workerId, startDate, endDate, groupBy = "month", userId } = params;
    ...
    const summary = {
      ...
      recentActivity: assignments
        ...
        .map(
          (
            /** @type {{ assignmentDate: any; luwangCount: any; status: any; pitak: { bukid: { name: any; }; location: any; }; }} */ a,
          ) => ({
    ...
    assignments.forEach(
      (
        /** @type {{ pitak: { bukid: { name: string; }; location: string; }; luwangCount: any; }} */ assignment,
      ) => {
        ...
        // @ts-ignore
        summary.byBukid[bukidName].totalLuwang += parseFloat(
        ...
        // @ts-ignore
        summary.byPitak[pitakLocation].totalLuwang += parseFloat(
          assignment.luwangCount || 0,
        );
      },
    );
    ...
    if (groupBy && assignments.length > 0) {
      ...
      assignments.forEach(
        (
          /** @type {{ assignmentDate: any; createdAt: any; luwangCount: any; status: string; }} */ assignment,
        ) => {
          let key;
          ...
          // @ts-ignore
          groupedAssignments[key].totalLuwang += parseFloat(
            assignment.luwangCount || 0,
          );
          // @ts-ignore
          if (assignment.status === "completed")
            groupedAssignments[key].completed++;
          // @ts-ignore
