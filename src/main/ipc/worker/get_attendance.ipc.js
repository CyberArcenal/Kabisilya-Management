...
module.exports = async function getWorkerAttendance(params = {}) {
  try {
    // @ts-ignore
    // @ts-ignore
    const { workerId, month, year, userId } = params;
    ...
    assignments.forEach((assignment) => {
      ...
      if (dayIndex >= 0 && dayIndex < daysInMonth) {
        ...
        // @ts-ignore
        attendance[dayIndex].totalLuwang += parseFloat(
    ...
    const summary = {
      ...
      daysOff: attendance.filter((day) => !day.isWeekend && !day.hasWork)
        .length,
      weekendDaysWorked: attendance.filter(
        (day) => day.isWeekend && day.hasWork,
      ).length,
      ...
      averageLuwangPerDay:
        attendance.filter((day) => day.hasWork).length > 0
          ? attendance.reduce((sum, day) => sum + day.totalLuwang, 0) /
            attendance.filter((day) => day.hasWork).length
          : 0,
      attendanceRate:
        (attendance.filter((day) => !day.isWeekend && day.hasWork).length /
          attendance.filter((day) => !day.isWeekend).length) *
        100,
    };
    ...
    attendance.forEach((day, index) => {
      ...
      if (day.dayOfWeek === "Sat" || index === attendance.length - 1) {
        ...
        // @ts-ignore
        const weekLuwang = currentWeek.reduce(
          (sum, d) => sum + d.totalLuwang,
          0,
        );
        // @ts-ignore
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const pastMonthName = pastMonth.toLocaleString("default", {
        month: "long",
      });
