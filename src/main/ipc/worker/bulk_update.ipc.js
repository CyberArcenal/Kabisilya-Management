...
module.exports = async function bulkUpdateWorkers(
  params = {},
  queryRunner = null,
) {
  ...
  try {
    ...
    for (const update of updates) {
      ...
      if (!id) {
        results.errors.push(
          // @ts-ignore
          `Missing worker ID for update: ${JSON.stringify(updateData)}`,
        );
        continue;
      }
      ...
      try {
        ...
        if (updateData.email && updateData.email !== worker.email) {
          ...
          if (emailExists) {
            results.errors.push(
              // @ts-ignore
              `Worker ID ${id}: Email ${updateData.email} already exists`,
            );
            continue;
          }
        }

        // Update worker fields (removed financial fields)
        if (updateData.name !== undefined) worker.name = updateData.name;
        if (updateData.contact !== undefined)
          worker.contact = updateData.contact;
        if (updateData.email !== undefined) worker.email = updateData.email;
        if (updateData.address !== undefined)
          worker.address = updateData.address;
        if (updateData.status !== undefined) worker.status = updateData.status;
        if (updateData.hireDate !== undefined)
          worker.hireDate = new Date(updateData.hireDate);

        // Note: Removed update for totalDebt, totalPaid, currentBalance
