...
// @ts-ignore
module.exports = async (/** @type {any} */ bukidId, /** @type {any} */ status, /** @type {any} */ userId) => {
  try {
    ...
    const detailedPitaks = pitaks.map(pitak => {
      ...
      return {
        ...
        assignments: {
          ...
          pending: assignments.assignmentCount - assignments.completedAssignments
        },
        payments: {
          total: payments.paymentCount,
          totalNetPay: payments.totalNetPay,
          totalGrossPay: payments.totalGrossPay,
          averageNetPay: payments.paymentCount > 0 ? payments.totalNetPay / payments.paymentCount : 0
        },
        utilization: {
          rate: utilizationRate,
          capacityUsed: assignments.totalLuWangAssigned,
          capacityRemaining: totalLuWangCapacity - assignments.totalLuWangAssigned
        },
        efficiency: {
          revenuePerLuWang: assignments.totalLuWangAssigned > 0 
            ? payments.totalNetPay / assignments.totalLuWangAssigned 
            : 0,
          assignmentsPerPayment: payments.paymentCount > 0 
    ...
    const summary = detailedPitaks.reduce((acc, pitak) => {
      acc.totalPitaks++;
      ...
      if (pitak.status === 'harvested') acc.harvestedPitaks++;
    ...
    // Calculate averages
    // @ts-ignore
    summary.averageLuWang = summary.totalPitaks > 0 ? summary.totalLuWangCapacity / summary.totalPitaks : 0;
    // @ts-ignore
    summary.averageUtilization = summary.totalLuWangCapacity > 0 
      ? (summary.totalLuWangAssigned / summary.totalLuWangCapacity) * 100 
      : 0;
    // @ts-ignore
    summary.averageRevenuePerPitak = summary.totalPitaks > 0 ? summary.totalRevenue / summary.totalPitaks : 0;
    ...
    detailedPitaks.forEach(pitak => {
      ...
      // @ts-ignore
      bukidGroups[bukidName].totalLuWangAssigned += pitak.assignments.totalLuWangAssigned;
      // @ts-ignore
    ...
    const bukidSummaries = Object.values(bukidGroups).map(group => ({
      ...group,
      utilizationRate: group.totalLuWangCapacity > 0 
        ? (group.totalLuWangAssigned / group.totalLuWangCapacity) * 100 
        : 0,
      averageRevenuePerPitak: group.pitakCount > 0 ? group.totalRevenue / group.pitakCount : 0
    }));
