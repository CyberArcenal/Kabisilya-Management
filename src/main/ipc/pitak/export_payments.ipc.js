...
module.exports = async (
  /** @type {{ pitakId: any; dateRange?: {} | undefined; userId: any; }} */ params,
) => {
  try {
    const { pitakId, dateRange = {}, userId } = params;
    ...
    const csvData = payments.map(
      (
        ...
        "Debt Deduction": parseFloat(payment.totalDebtDeduction || 0).toFixed(
          2,
        ),
        ...
        "Updated Date": payment.updatedAt
          ? payment.updatedAt.toISOString().split("T")[0]
          : "",
      }),
    );
    ...
    const summary = payments.reduce(
      (
        /** @type {{ totalPayments: number; totalGrossPay: number; totalNetPay: number; totalDeductions: number; completed: number; }} */ sum,
        /** @type {{ grossPay: string; netPay: string; totalDebtDeduction: string; otherDeductions: string; status: string; }} */ payment,
      ) => {
        sum.totalPayments++;
