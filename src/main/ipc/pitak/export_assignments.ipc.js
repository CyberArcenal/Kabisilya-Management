...
module.exports = async (
  ...
  try {
    ...
    const csvData = assignments.map((assignment) => ({
      ...
      // @ts-ignore
      "Worker Contact": assignment.worker
      ...
      // @ts-ignore
      "Updated Date": assignment.updatedAt
        ? assignment.updatedAt.toISOString().split("T")[0]
        : "",
    }));
    ...
    const summary = assignments.reduce(
      (
        /** @type {{ totalAssignments: number; totalLuWang: number; completed: number; active: number; cancelled: number; }} */ sum,
