...
const Pitak = require("../../../entities/Pitak");
const { validateWorkers, validatePitak, findAlreadyAssigned } = require("./utils/assignmentUtils");
...
module.exports = async (params, queryRunner) => {
  try {
    ...
    for (let i = 0; i < assignments.length; i++) {
      ...
      // Validate worker
      if (assignment.workerId) {
        const workerCheck = await validateWorkers(workerRepo, [assignment.workerId]);
